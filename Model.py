...
class Model: 
	...
	def setupCNN(self):
		...
		kernelVals = [5, 5, 3, 3, 3] # convolution operation with the respective kernel sizes for the 5 layers
		featureVals = [1, 32, 64, 128, 128, 256] # number of features each layer can identify
		strideVals = poolVals = [(2,2), (2,2), (1,2), (1,2), (1,2)] # stride values/ shifts at which the kernel will process the input
		numLayers = len(strideVals) # number of CNN layers we are using in the model = 5
		...
		for i in range(numLayers):
			...
			relu = tf.nn.relu(conv_norm) # using the non linear RELU as an activation function
			pool = tf.nn.max_pool(relu, (1, poolVals[i][0], poolVals[i][1], 1), (1, strideVals[i][0], strideVals[i][1], 1), 'VALID')
			# pooling the input at each layer to reduce the size of the input for the next layer
	...
	def setupRNN(self):
		...
		numHidden = 256 # basic cells which is used to build RNN
		cells = [tf.contrib.rnn.LSTMCell(num_units=numHidden, state_is_tuple=True) for _ in range(2)] # 2 layers and the LSTM implementation
		stacked = tf.contrib.rnn.MultiRNNCell(cells, state_is_tuple=True) # stack basic cells
		((fw, bw), _) = tf.nn.bidirectional_dynamic_rnn(cell_fw=stacked, cell_bw=stacked, inputs=rnnIn3d, dtype=rnnIn3d.dtype)	# bidirectional RNN				
		concat = tf.expand_dims(tf.concat([fw, bw], 2), 2)	# BxTxH + BxTxH -> BxTx2H -> BxTx1X2H				
